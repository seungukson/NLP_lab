...
class MultiHeadAttention(tf.keras.Model):
    ...
    def scaled_dot_product_attention(self, query, key, value, masked=False):
        ...
        if masked:
            ...
            tril = tf.linalg.LinearOperatorLowerTriangular(diag_vals).to_dense() #하 삼각행렬

            masks = tf.tile(tf.expand_dims(tril, 0), [tf.shape(outputs)[0], 1, 1])#요거랑아랫줄: 하삼각행렬제외 -무한대
            paddings = tf.ones_like(masks) * (-2 ** 32 + 1)
            #의도: 1cycle:그 2cycle:그 사람도, 3cycle: 그사람도 그럴.
            outputs = tf.where(tf.equal(masks, 0), paddings, outputs)

        attention_map = tf.nn.softmax(outputs) # 하삼각함수값이 0이된다?

        return tf.matmul(attention_map, value)

    def call(self, query, key, value):
        query = self.query_dense(query) # 쿼리를 날려서 키와 비교하며 밸류(집중도)를 뽑아냄
    ...
    for i in range(loop_count):
        with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE):
            if i > 0:#이전에 예측한 값
                ...
                output = features['output'] #처음루프에서 동작, <start> 가 들어감.
