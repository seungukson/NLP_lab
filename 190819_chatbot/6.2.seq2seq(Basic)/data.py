...
FILTERS = "([~.,!?\"':;)(])"
PAD = "<PADDING>"   # 짧은 문장은 뒷부분을 padding
STD = "<START>"     # decorder에서 문장의 시작과 끝을 표시
END = "<END>"
UNK = "<UNKNOWN>"   # voca에서 없는단어
...
def load_data():
    ...
    train_input, eval_input, train_label, eval_label = \
        train_test_split(question, answer, test_size=0.1, random_state=42) #10%가 평가용.
    return train_input, train_label, eval_input, eval_label # <학습용>/<평가용>


# 형태소 분석
# 감성분석이나 문서 분류에는 형태소 분석이 필요하다. 하지만 챗봇에 형태소 분석을 적용하면
# 형태소로 답변하게 된다. 따라서 챗봇에는 형태소 분석을 적용하기 곤란하다.
# 챗봇에 형태소 분석을 적용하려면 답변시 형태소를 이용하여 문장을 generation하는 기술;아직없음 이
...
# 디코더 입력과 타겟에는 앞 뒤에 STD, END가 들어간다.
# 예시:
# DEFINES.max_sequence_length = 10 인 경우

#3개의 데이터가 필요하다.
# 인코더 입력 : "가끔 궁금해" -> [9310, 17707, 0, 0, 0, 0, 0, 0, 0, 0]
# 디코더 입력 : "그 사람도 그럴 거예요" -> [STD, 20190, 4221, 13697, 14552, 0, ...] #sequence_index
# 디코더 타켓 : [20190, 4221, 13697, 14552, END, 0, ...]
def data_processing(value, dictionary, pType):  # pType: 인코더?디코더입력?디코더타겟?
    # 형태소 토크나이징 사용 유무
    if DEFINES.tokenize_as_morph:  # false in configs.py
    ...
    for sequence in value:
        ...
        for word in sequence.split():
            # word가 딕셔너리에 없으면 UNK (out of vacabulary)를 넣는다. unknown = 3
...
def rearrange(inputX, outputX, targetY):
    ...
    return features, targetY # {"input":input_encoder,"output":output_output_decoder},target_decoder
